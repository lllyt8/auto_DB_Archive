...
4. If using `${VAR}` placeholders in config, export those environment variables in the shell profile used by cron.
...
## 6) Weekly cron (Sunday 02:10 EST)
Add to `crontab -e`:
```cron
